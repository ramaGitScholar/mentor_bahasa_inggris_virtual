...
class ChatRepository:
    ...
    def save_message(self, user_id:int, role: Literal["user", "model"], message_text: str): # role -> user/model
...
# chat_repository = ChatRepository() # instansiasi
    ...
    def save_user(self, user_id: int, username: str, chat_id: int):
        exist_user = (
            self.supabase.table("chat_users").select("user_id").eq("user_id",user_id).execute()
        )
    ...
    def get_last_artifact_by_user_id(self, user_id: int):
        result = (
            ...
            .select("artifact")
            .eq("user_id", user_id)
            .not_.is_("artifact", "null") # ambil artifak yang tidak kosong
            .order("created_at", desc=True)
            .limit(1)
            .execute()
        )
