...
# kode menyediakan tools dan model menjadi pengambil keputusan akan fungsi apa yang dipanggil
...
class LeadAgent:
    def __init__(self):
        ...
        # tools istilah yang sering digunakan oleh lead agent dalam memanfaatkan fungsi yang telah didefinisikan (automatic function calling)
        self.tools: list = [
    ...
    # pythonic way: nama function dengan awalan underscore, akan dipanggil secara private        
    def _load_history(self, user_id: int):
        ...
        # list comprehension (dari database menjadi format yang dapat dipahami llm)
        return [
            types.Content(
                role=row["role"],
                parts=[
    ...
    def handle_send_voice(self, user_id: int, voice_file_path: str):
        ...
        evaluation_speaking_result = services.evaluate_speaking(voice_file_path=voice_file_path)
